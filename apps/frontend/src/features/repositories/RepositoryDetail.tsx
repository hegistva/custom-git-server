...
export function RepositoryDetail({ owner, name }: { owner: string; name: string }) {
  ...
  if (isError || !repo)
    return <p role="alert">Failed to load repository details or it does not exist.</p>;
    ...
    <section>
      ...
      >
        <div>
          ...
          {repo.description && (
            <p style={{ fontSize: '1.1rem', color: '#555' }}>{repo.description}</p>
          )}
        </div>
        {user?.username === owner && (
          <div>
      ...
      >
        ...
        <p style={{ fontSize: '0.85rem', color: '#666' }}>
          Note: You must use a Personal Access Token (PAT) for HTTPS authentication. Your username
          is <strong>{user?.username}</strong> and password is your PAT.
        </p>
