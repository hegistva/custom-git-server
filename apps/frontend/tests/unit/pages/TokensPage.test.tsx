...
describe('TokensPage', () => {
  ...
  it('renders token list from api', async () => {
    vi.mocked(tokensApi.list).mockResolvedValueOnce([
      { id: '1', label: 'My test pat', tokenPrefix: 'abcdefgh', createdAt: '2025-01-01T00:00:00.000Z', expiresAt: null, lastUsedAt: null, revokedAt: null }
    ])
  ...
  it('add token form validates and submits', async () => {
    ...
    expect(tokensApi.generate).toHaveBeenCalledWith(
      { label: 'New PAT' },
      expect.anything()
    )
